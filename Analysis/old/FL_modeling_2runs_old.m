...
mrDataPath_1 = [rootDir '/' 'Subject_Data' '/' 'sj' num2str(sjNum) '/' 'data.functional.FL_b1' ];
mrDataPath_2 = [rootDir '/' 'Subject_Data' '/' 'sj' num2str(sjNum) '/' 'data.functional.FL_b2' ];
...
tmp1 = load([trialData '/' sprintf('sj%d_se%02d_Trial_Data_FL_SPM.mat',sjNum,1)]); % run1
tmp2 = load([trialData '/' sprintf('sj%d_se%02d_Trial_Data_FL_SPM.mat',sjNum,2)]); % run2

%% load michelle's basicBatch mat file (already contains model/stats stuff for rwls)
load([batchFilePath '/' 'basicBatch.mat'])

%% get list of pre-processed nii files in the dirs
d=dir([mrDataPath_1 '/' 'uf*']);
for i=1:length(d)
    thisNii_1(i,:) = [mrDataPath_1 '/' d(i).name ',1']; % why the ',1' needed?
end

d=dir([mrDataPath_2 '/' 'uf*']);
for i=1:length(d)
    thisNii_2(i,:) = [mrDataPath_2 '/' d(i).name ',1']; % why the ',1' needed?
end

% combine the dirs of nii files
thisNii = [thisNii_1;thisNii_2];
