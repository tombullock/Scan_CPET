...
function model_visHRF_bothSessions(sjNum)
...
for thisSession=1:2
    ...
    clear mrDataPath trialData destDir d thisNii files
    ...
    %% put all betas in combined pre/post dir
    ...
    for i=1:12
        ...
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(thisSession).cond(i).onset=thisOnset;
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(thisSession).cond(i).name=thisName;
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(thisSession).cond(i).duration= .5; % roughly
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(thisSession).cond(i).pmod = struct('name', {}, 'param', {}, 'poly', {});
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(thisSession).cond(i).orth = 1;
        matlabbatch{1}.spm.tools.rwls.fmri_rwls_spec.sess(thisSession).cond(i).tmod = 0;
    end
